    ...
    while num_char < len(secret):
        ...
        if contains_char(secret, guess[num_char]) == True:
            ...
            if guess[num_char] == secret[num_char]:
                ...
                # (for example: if a is the second letter in both the guess word
                # and secret word)
            else:
                emoji_str += YELLOW_BOX
            num_char += 1
            # the corresponding else statement adds a Yellow box emoji to the emoji
            # string variable if the guess character is in the secret word but in
            # a different place then it adds one to the index variable to iterate
            # through each character of the word
        else:
            emoji_str += WHITE_BOX
            num_char += 1
            # an else statement is used if the guess character is not found in
            # the secret word, meaning if the contains_char is == False
    print(emoji_str)
    return emoji_str
...
def main(secret: str) -> None:
    ...
    while N <= 6:
        print(f"=== Turn {N}/6 ===")
        # creates a while loop that first prints a statement
        # saying which turn the user is on
        guess: str = input_guess(len(secret))
        # defines a local variable that takes the value of the input of the
        ...
        if guess == secret:
            ...
            return
            # the if statement checks to see if the guessed word is the same as the
            # secret word if that is true and the inputted guess matches the secret
            # word, it prints a statement saying that the user won in a specific number
            # of turns using the N variable that was used to count turns.
